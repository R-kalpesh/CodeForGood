...
//        arr.addAll(Arrays.asList(new Integer[]{90,80,30,20,50,60,70}));
        ...
        int N=4;
        int one=0,zero=0;
        obj.nBitBinary(op,N,one,zero,vec);
        System.out.println(vec);
    }

    void nBitBinary(String op,int N,int one,int zero,Vector<String> vec){
        if(N==0){
            vec.add(op);
            return;
        }
        String op1=op+"1";
        nBitBinary(op1,N-1,one+1,zero,vec);
        if(zero<one){
            String op2=op+"0";
            nBitBinary(op2,N-1,one,zero+1,vec);
        }
        return;
    }
